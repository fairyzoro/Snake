...
public class Snake extends Canvas implements Runnable {//Class of the game
	...
	public static void main(String[] args) {//main method
		...
		rainbow = new int[10000];
		for(int i = 0; i < rainbow.length-7; i+=7){
			rainbow[0+i] = 0xff0000;
			rainbow[1+i] = 0xffa500;
			rainbow[2+i] = 0xffff00;
			rainbow[3+i] = 0x00ff00;
			rainbow[4+i] = 0x0000ff;
			rainbow[5+i] = 0x4b0082;
			rainbow[6+i] = 0xee82ee;
		}
				...
				if(!death){
					...
					loop:{
						for(int i = 0; i < parts.size(); i++){
							...
							pixels[parts.get(i).getX() + parts.get(i).getY() * width] = (color << i) | 0x0000ff; //set snakeparts to varying shades of blue and grey, could also use info from rainbow array
